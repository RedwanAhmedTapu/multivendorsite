...
export function VendorOffersManagement() {
  ...
  const [isCreateVoucherDialogOpen, setIsCreateVoucherDialogOpen] =
    useState(false);
  ...
  const { data: offersData, isLoading: offersLoading } =
    useGetVendorOffersQuery(filters);
  const { data: permissionsData } = useGetVendorPermissionsQuery(
    vendorId ?? ""
  );
  ...
  const handleCreateOffer = () => {
    router.push("/vendor-dashboard/offers/create-offer");
  };
            ...
            <div className="flex items-center gap-2">
              {permissions && (
                <Badge variant="outline" className="text-xs">
                  ...
                  {permissions.maxActiveOffers} Active
                </Badge>
              )}
            </div>
          </div>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid w-full grid-cols-5">
              ...
              <TabsTrigger value="pending">
                Pending Approval
                {offers.filter(
                  (o) => o.createdByType === "VENDOR" && !o.isActive
                ).length > 0 && (
                  <Badge
                    variant="destructive"
                    className="ml-2 h-4 w-4 p-0 text-[10px]"
                  >
                    {
                      offers.filter(
                        (o) => o.createdByType === "VENDOR" && !o.isActive
