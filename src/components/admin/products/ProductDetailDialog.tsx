            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-md hover:shadow-lg transition-shadow duration-300">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-sm font-semibold bg-orange-50 px-3 py-2 rounded-lg border border-orange-200 text-orange-900">
            ...
            <Tabs defaultValue="variants" className="w-full">
                        ...
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            <div className="flex items-center gap-2 mb-1.5">
                              ...
                              <h5 className="font-bold text-base text-gray-900">{variant.name}</h5>
                            </div>
                            <p className="text-xs text-gray-600 font-mono bg-gray-50 px-2 py-1 rounded inline-block border border-gray-200">{variant.sku}</p>
                          </div>
                          <Badge variant={variant.stock > 0 ? 'default' : 'destructive'} className="text-xs px-3 py-1 font-semibold">
              ...
              <TabsContent value="specs" className="space-y-4 mt-5">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="secondary" className="text-xs px-3 py-1 font-semibold">
                    {specifications.length} {specifications.length === 1 ? 'spec' : 'specs'}
                  </Badge>
                </div>
                {specifications.length === 0 ? (
                  ...
                  <div className="grid gap-3">
                    {specifications.map((spec, index) => (
                      <div key={spec.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 hover:border-blue-300 hover:shadow-sm transition-all duration-300">
                        <div className="flex items-center gap-3">
                          <span className="text-xs font-bold text-white bg-gray-500 px-2 py-1 rounded-md">#{index + 1}</span>
                          <span className="font-semibold text-sm text-gray-900">
                            {spec.specification?.name || 'Unknown'}
                          </span>
                        </div>
                        <span className="text-sm font-semibold text-gray-700 bg-white px-4 py-2 rounded-lg border border-gray-300">
              ...
              {/* Attributes Tab */}
              <TabsContent value="attributes" className="space-y-4 mt-5">
                <div className="flex items-center justify-between">
                  <h4 className="text-lg font-bold text-gray-900">Attribute Settings</h4>
                  <Badge variant="secondary" className="text-xs px-3 py-1 font-semibold">
                    {attributeSettings.length} {attributeSettings.length === 1 ? 'attribute' : 'attributes'}
                  </Badge>
                </div>
                {attributeSettings.length === 0 ? (
                  <div className="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                    <Tag className="h-12 w-12 text-gray-400 mx-auto mb-3" />
                    <p className="text-gray-500 font-medium">No attribute settings available</p>
                  </div>
                ) : (
                  <div className="grid gap-3">
                    {attributeSettings.map((setting, index) => (
                      <div key={setting.id} className="flex items-center justify-between p-4 bg-purple-50 rounded-lg border border-purple-200 hover:border-purple-400 hover:shadow-sm transition-all duration-300">
                        <div className="flex items-center gap-3">
                          <span className="text-xs font-bold text-white bg-purple-500 px-2 py-1 rounded-md">#{index + 1}</span>
                          <div>
                            <p className="font-semibold text-sm text-gray-900">
                              {setting.attribute?.name || 'Unknown'}
                            </p>
                            <p className="text-xs text-gray-500 font-mono mt-0.5">
                              ID: {setting.attributeId}
                            </p>
                          </div>
                        </div>
                        <Badge variant={setting.isVariant ? 'default' : 'secondary'} className="text-xs px-3 py-1 font-semibold">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">{new Date(product.createdAt).toLocaleString()}</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">{new Date(product.updatedAt).toLocaleString()}</p>
                  </div>
                </div>
              </div>
            </div>
