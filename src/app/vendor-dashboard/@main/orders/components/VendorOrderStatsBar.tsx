...
import { useGetVendorOrderStatisticsQuery } from "@/features/vendorOrderApi";
...
function StatCard({
  ...
  return (
    <div className="relative bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden hover:shadow-md transition-all duration-200 group">
      ...
      <div className="p-4">
        <div
          className={`inline-flex p-2 rounded-xl mb-3 ${accentColor
            .replace("-500", "-50")
            .replace("-400", "-50")
            .replace("-600", "-50")
            .replace("-700", "-50")}`}
...
export function VendorOrderStatsBar() {
  const { data, isLoading } = useGetVendorOrderStatisticsQuery();
  const s = data?.data;
  ...
  const totalOrders   = s?.total;
  const pendingOrders = (s?.byStatus.pending ?? 0) + (s?.byStatus.confirmed ?? 0);
  const processing    = (s?.byStatus.processing ?? 0) + (s?.byStatus.packaging ?? 0);
  const shipped       = s?.byStatus.shipped ?? 0;
  const delivered     = s?.byStatus.delivered ?? 0;
